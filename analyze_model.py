...
model, model_true, data, posterior_dict, posterior_path, data_corr = \
    au.get_best_model(model_folders, run_params['sorting_param'], use_test_data=run_params['use_test_data'],
...
model_sampled_irf_rms = au.ave_fun(model_sampled_irf[-window[0]:], axis=0)
...
measured_irf_rms[num_stim < run_params['num_stim_cutoff']] = np.nan
model_sampled_irf_rms[num_stim < run_params['num_stim_cutoff']] = np.nan
...
am.plot_irf_norm(model, measured_irf_rms, model_sampled_irf_rms, data_corr, cell_ids_chosen)
am.compare_irf_w_prediction(model, measured_irf_rms, model_sampled_irf_rms, data_corr, cell_ids_chosen)

am.plot_irf_traces(measured_irf, measured_irf_sem, posterior_irf, model_sampled_irf, cell_ids, cell_ids_chosen, window,
                   sample_rate=model.sample_rate, num_plot=5)

# if the data is not synthetic compare with the anatomy
if not is_synth:
    am.compare_irf_w_anatomy(model, measured_irf_rms, model_sampled_irf_rms, data_corr, cell_ids_chosen)

posterior_dict = am.plot_missing_neuron(model, data, posterior_dict, cell_ids_chosen, neuron_to_remove, force_calc=run_params['force_calc_missing_posterior'])
