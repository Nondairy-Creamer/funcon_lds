...
class Lgssm:
    ...
    def __init__(self, latent_dim, dtype=torch.float64, device='cpu', verbose=True):
        ...
        self.dynamics_input_weights_log_init = np.zeros(self.latent_dim) - 1
        self.dynamics_cov_log_init = np.zeros(self.latent_dim) - 1
        self.dynamics_bias_init = np.zeros(self.latent_dim)

        self.emissions_weights_init = 0.9 * np.eye(self.latent_dim)
        self.emissions_input_weights_log_init = np.zeros(self.latent_dim) - 1
        self.emissions_cov_log_init = np.zeros(self.latent_dim) - 1
        self.emissions_bias_init = np.zeros(self.latent_dim)

        self.dynamics_weights = torch.tensor(self.dynamics_weights_init, device=self.device,
                                             dtype=self.dtype, requires_grad=True)
        self.dynamics_input_weights_log = torch.tensor(self.dynamics_input_weights_log_init, device=self.device,
                                                       dtype=self.dtype, requires_grad=True)
        self.dynamics_cov_log = torch.tensor(self.dynamics_cov_log_init, device=self.device,
                                             dtype=self.dtype, requires_grad=True)
        self.dynamics_bias = torch.tensor(self.dynamics_bias_init, device=self.device,
                                          dtype=self.dtype, requires_grad=True)

        self.emissions_weights = torch.tensor(self.emissions_weights_init, device=self.device,
                                              dtype=self.dtype, requires_grad=True)
        self.emissions_input_weights_log = torch.tensor(self.emissions_input_weights_log_init, device=self.device,
                                                        dtype=self.dtype, requires_grad=True)
        self.emissions_cov_log = torch.tensor(self.emissions_cov_log_init, device=self.device,
                                              dtype=self.dtype, requires_grad=True)
        self.emissions_bias = torch.tensor(self.emissions_bias_init, device=self.device,
                                           dtype=self.dtype, requires_grad=True)
    ...
    def randomize_weights(self, max_eig_allowed=0.8, init_std=0.1, cov_log_offset=-1, random_seed=None):
        ...
        self.emissions_weights_init = rng.standard_normal((self.latent_dim, self.latent_dim))
        max_eig_in_mat = np.max(np.abs(np.linalg.eigvals(self.emissions_weights_init)))
        self.emissions_weights_init = max_eig_allowed * self.emissions_weights_init / max_eig_in_mat

        self.dynamics_input_weights_log_init = init_std * rng.standard_normal(self.latent_dim) + cov_log_offset
        self.dynamics_cov_log_init = init_std * rng.standard_normal(self.latent_dim) + cov_log_offset
        self.dynamics_bias_init = init_std * rng.standard_normal(self.latent_dim)

        self.emissions_input_weights_log_init = init_std * rng.standard_normal(self.latent_dim) + cov_log_offset
        self.emissions_cov_log_init = init_std * rng.standard_normal(self.latent_dim) + cov_log_offset
        self.emissions_bias_init = init_std * rng.standard_normal(self.latent_dim)
        ...
        self.emissions_cov_log = torch.tensor(self.emissions_cov_log_init, device=self.device,
                                              dtype=self.dtype, requires_grad=True)
        self.emissions_bias = torch.tensor(self.emissions_bias, device=self.device,
                                           dtype=self.dtype, requires_grad=True)
    ...
    def set_device(self, new_device):
        ...
        self.dynamics_input_weights_log = self.dynamics_input_weights_log.to(new_device)
        self.dynamics_cov_log = self.dynamics_cov_log.to(new_device)
        self.dynamics_bias = self.dynamics_bias.to(new_device)

        self.emissions_weights = self.emissions_weights.to(new_device)
        self.emissions_input_weights_log = self.emissions_input_weights_log.to(new_device)
        self.emissions_cov_log = self.emissions_cov_log.to(new_device)
        self.emissions_bias = self.emissions_bias.to(new_device)
    ...
    def fit_gd(self, emissions_list, inputs_list, learning_rate=1e-2, num_steps=50):
        ...
        model_params = \
            [self.dynamics_weights, self.dynamics_input_weights_log, self.dynamics_cov_log, self.dynamics_bias,
             self.emissions_weights, self.emissions_input_weights_log, self.emissions_cov_log, self.emissions_bias]
    ...
    def loss_fn(self, emissions, inputs, init_mean, init_cov, nan_fill=1e8):
        ...
        return -torch.mean(loss) / emissions.numel()
    ...
    def sample(self, num_time=100, num_data_sets=None, nan_freq=0.0, random_seed=None):
        ...
        dynamics_input_weights = np.diag(np.exp(self.dynamics_input_weights_log.detach().cpu().numpy().copy()))
        dynamics_cov = np.diag(np.exp(self.dynamics_cov_log.detach().cpu().numpy().copy()))
        dynamics_bias = self.dynamics_bias.detach().cpu().numpy().copy()

        emissions_weights = self.emissions_weights.detach().cpu().numpy().copy()
        emissions_input_weights = np.diag(np.exp(self.emissions_input_weights_log.detach().cpu().numpy().copy()))
        emissions_cov = np.diag(np.exp(self.emissions_cov_log.detach().cpu().numpy().copy()))
        emissions_bias = self.emissions_cov_log.detach().cpu().numpy().copy()
        ...
        for t in range(1, num_time):
            ...
            latents[:, t, :] = (dynamics_weights[None, :, :] @ latents[:, t - 1, :, None])[:, :, 0] + dynamics_input_weights[None, :] * inputs[:, t, :] + dynamics_noise
    ...
    def _lgssm_filter(self, emissions, inputs, init_mean, init_cov, nan_fill=1e8):
        ...
        inputs_weights = torch.exp(self.dynamics_inputs_weights_log)
