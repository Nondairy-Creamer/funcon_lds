...
class Lgssm:
    ...
    def lgssm_filter_torch(self, emissions, inputs, emissions_offset, init_mean, init_cov, memmap_cpu_id=None):
        """Run a Kalman filter to produce the marginal likelihood and filtered state estimates.
        adapted from Dynamax.
        ...
        dynamics_inputs = self.get_lagged_data_torch(inputs, self.dynamics_input_lags)
        emissions_inputs = self.get_lagged_data_torch(inputs, self.emissions_input_lags)
    ...
    @staticmethod
    def get_lagged_data_torch(data, lags, add_pad=True):
        num_time, num_neurons = data.shape

        if add_pad:
            final_time = num_time
            pad = torch.zeros((lags - 1, num_neurons), device=data.device)
            data = torch.cat((pad, data), dim=0)
        else:
            final_time = num_time - lags + 1

        lagged_data = torch.zeros((final_time, 0), device=data.device)
